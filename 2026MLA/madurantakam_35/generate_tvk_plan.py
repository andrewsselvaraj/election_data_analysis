...
TEMPLATE_PATH = "tvk_plan_template.html"
OUT_PATH = "madurantakam_35_tvk_plan.html"

# candidate_name -> (party_code, display party name), as specified by the user
PARTY_MAP = {
...
BATTLEGROUND_THRESHOLD = 15.0  # percentage points behind the leader
...
with open(CSV_PATH, encoding="utf-8") as f:
    ...
    for row in reader:
        ...
        if key is None:
            ...
            key = len(booths) - 1
        rec = booths[key]
        party_code, party_display = PARTY_MAP.get(row["candidate_name"], (None, row["party_name"]))
...
# tag duplicate booth_no records (Form 20 lists booth 218 twice)
...
for b in booths:
    ...
    b["tvk_pct"] = tvk["pct"]
    b["dmk_votes"] = cand_by_name[DMK_NAME]["votes"]
    b["aiadmk_votes"] = cand_by_name[AIADMK_NAME]["votes"]
    ...
    if total == 0:
        ...
        b["status"] = "stronghold"
        b["gap_pct"] = -cushion  # negative = TVK is ahead by this many points
        b["priority"] = 1000 + cushion  # most vulnerable (smallest cushion) sorts first within group
        ...
        if gap <= BATTLEGROUND_THRESHOLD:
            ...
            b["priority"] = 0 + gap  # closest-to-flip sorts first
